    ...
    try {
        if (cmd == "create") {
            ...
            if ((argc != 2 && argc != 3) ||
                ...
                std::cerr << "Usage: run <index> [-a|--attach]" << std::endl;
                return;
            }

            docker.run(std::stoul(argv[1]), argc == 3);
        } else if (cmd == "remove") {
            if (argc < 2) {
            ...
            if (argc != 2){
                std::cerr << "Usage: attach <index>" << std::endl;
                return;
            }
            docker.attach(std::stoul(argv[1]), false);
        } else if (cmd == "help") {
            std::cout << "Available commands:\n"
                    << "  create <config_path>      - Create a new container\n"
                    << "  list                      - List all containers\n"
                    << "  run <index> [-a|--attach] - Run a container\n"
